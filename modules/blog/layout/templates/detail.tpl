...
<div id="blogDetail">
	...
	<!-- Article -->
	{option:item}
	<article class="full articleBlog">

		<div class=" articleContent">
			<div class="articleBlogTitle">
		<h1>{$item.title}</h1>
		<p class="date ">{$msgWrittenBy|ucfirst|sprintf:{$item.user_id|usersetting:'nickname'}} {$lblOn}
			...
			{option:item.image}
			<img src="{$FRONTEND_FILES_URL}/blog/images/source/{$item.image}" alt="{$item.image}" class="articleImageMain">
			{/option:item.image}
				...
				<article id="comment-{$comments.id}" class="comment" itemprop="comment" itemscope itemtype="http://schema.org/UserComments">
					<meta itemprop="discusses" content="{$item.title}" />
						<p class="commentAuthor" itemscope itemtype="http://schema.org/Person">
							...
							{/option:comments.website}
							{$lblWrote}
							<time itemprop="commentTime" datetime="{$comments.created_on|date:'Y-m-d\TH:i:s'}">{$comments.created_on|timeago}
							</time>
						</p>
